...
import 'package:sistema_escolar_prepa/models/rating.dart';
...
import '../Services/rating_sevices.dart';
...
class _ScreenListStudentByGradesState extends State<ScreenListStudentByGrades> {
  ...
  // late List<StudentByGrades>? ratings=[];
  @override
  Widget build(BuildContext context) {
    ...
    // ratings.insert(0, ['fsf','fsdf','fdsfds','fsdf','fsf','fsdf','fdsfds']);
    ...
    //   // matrix.add(ratings[i]);
    //   matrix.add(["${ratings[i].student.name}  ${ratings[i].student.lastName}  ${ratings[i].student.secondName}"]);
    // }
    ...
    return Scaffold(
      floatingActionButton: FloatingActionButton.extended(
          onPressed: () async {
            // si esta vacion most4rar un mensaje de que no hayarchivos
                ...
                builder: (BuildContext context) {
                  ...
                  title: Text('Error'),
                    content: SizedBox(
                      height: 170,
                      child: const Center(
                        child: Text('No hay alumnos con estas calificaciones para generar la lista'),
                      ),
                    ), 
                    actions: [
                      Center(
                    ...
                    return pw.Column(
                      ...
                      children: [
                        pw.Center(
                            child: pw.Column(
                          children: [
                            pw.Text('Escualea Preparatoria'),
                            pw.Text('Rafael Pascacion Gamboa'),
                            pw.Text('Nivel Medio Superios'),
                          ],
                        )),
                        pw.SizedBox(
                          height: 100,
                        ...
                        pw.Text(
                            'Lista de las mejroes estudiaens del primer parcial'),
                        pw.SizedBox(
                          height: 20,
                        ),
                        // pw.ListView(
                        ...
                        //     pw.Container(
                        //       width: 30,
                        //       child: pw.Text(ratings[index].parcial1.toString()),
                        //     ),
                        //   ]);
                        // })),

                        // este es para crear tablas
// pw.Row(children: []),
                        pw.Table.fromTextArray(
                            context: context,
                            data:

                                //  hay que craer una matix de datos
                                List.generate(ratings!.length, (index) {
                              // ratings.insert(0, ['SFSFSD','FSDFSDF','ASFASDF','FFSF']);
                              return <String>[
                                // <String>['INDICE', 'NOMBRE', 'datos']
                                '${index}',
                                "${ratings![index].student.name}  ${ratings![index].student.lastName}  ${ratings![index].student.secondName}",
                                '${ratings![index].parcial1}',
                                '${ratings![index].parcial2}',
                                '${ratings![index].parcial3}',
                                '${ratings![index].parcial3}'
                              ];
                            }).toList())
                      ],
...
// OpenFile.open("/sdcard/example.txt");
      ...
      appBar: AppBar(
        title: const Text('Lista de estudianes'),
      ),
