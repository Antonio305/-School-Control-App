...
import '../widgets/utils/style_elevated_button.dart';
    ...
    return showDialog<String>(
      ...
      builder: (BuildContext context) {
        ...
        return AlertDialog(
            ...
            content: SizedBox(
              width: 
              Platform.isWindows || Platform.isMacOS ?               size.width * 0.3 : 
              size.width * 0.4,
...
void showSubject(BuildContext context, int partial, int option) {
  showDialog(
    ...
    builder: (BuildContext context) {
      ...
      return AlertDialog(
        title: const Text('SELECCIONA LA MATERIA'),
        content: SizedBox(
...
void showGroup(
  ...
  showDialog(
    context: context,
    builder: (BuildContext context) {
      List<String> listIdGenerations(List<Generation> generations) {
        List<String> idGeneration = [];

        for (var generation in generations) {
          idGeneration.add(generation.uid);
        }

        // return the last three generations
        List<String> lastThreeGeneration =
            // idGeneration.sublist(idGeneration.length - 3);
            idGeneration.sublist(0, 3);

        // return idGeneration;
        return lastThreeGeneration;
      }
                    ...
                    onPressed: () async {
                      ...
                      final generationServices =
                          Provider.of<GenerationServices>(context,
                              listen: false);
                      List<Generation> yes = generationServices.generations;
                      // generaciones

                      final listGeneration = listIdGenerations(yes);
                      print('LISTA DE LAS ULTIMAS GENERACIONES');
                      print(listGeneration);

// crear un switch para saber cual funtion hay que ejecutar
                      switch (option) {
                        case 1:
                          print('PARTIAL ONE');
